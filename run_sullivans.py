...
#------------------------------------------------------------------------------
EVENT_OPTIONS = ['50pc','20pc','10pc','05pc','02pc', '01pc','500y']
...
parser.add_argument('-du', '--duration', type=float, default=def_duration, help='finaltime')  
parser.add_argument('-ys', '--yieldstep', type=float, default=def_yieldstep, help='yieldstep')
parser.add_argument('-nu', '--mannings', type=float, default=def_manning, help='Channel Mannings')
parser.add_argument('-e', '--event', type=str, default=def_event, help='Rain event')   
parser.add_argument('-alg', type=str, default = def_alg, help='flow algorithm')
    

args = parser.parse_args()

alg = args.alg
RAIN_EVENT = args.event
...
if ENTIRE_CATCHMENT == True:
    ...
    Drain_Barry_Polygon    = su.readShp_1PolyGeo(join('05_SHAPEFILES','Drain_Barry_Drive.shp'), dropLast=False)
    Drain_Library_Polygon  = su.readShp_1PolyGeo(join('05_SHAPEFILES','Drain_Library.shp'), dropLast=False)


    interior_regions = [[Catchment_Rain_Polygon,300],
                        [Valley_Refine_Polygon1,75],
                        [ANU_Polygon,25],
                        [Drain_Barry_Polygon,25],
                        [Drain_Library_Polygon,25]]
    ...
    riverWall={ 'Barry_Embackment': PolylineToRiverwall(Barry_Embackment,565)}
    
    breaklines = [ Barry_Embackment, East_Bank, West_Bank]
...
Manning = su.readShp_1PolyGeo(join('05_SHAPEFILES','Manning.shp'), dropLast=False)
Union   = su.readShp_1PolyGeo(join('05_SHAPEFILES','Union.shp'), dropLast=False)

interior_holes = interior_holes + [Manning, Union]
...
if myid == 0:
    ...
    #-------------------------------------------------------------------------------
    # Set Elevation in specific locations
    #-------------------------------------------------------------------------------
    
    domain.set_quantity('elevation', 559.0, location='centroids',polygon=Fellows_Creek)
    domain.set_quantity('elevation', 559.5, location='centroids',polygon=Arts_Center_Creek)
    domain.set_quantity('elevation', 559.6, location='centroids',polygon=Union_Creek)
    ...
    domain.set_quantity('elevation', 562.0, location='centroids',polygon=Gods_Embackment)
    ...
    domain.set_quantity('friction', mannings, location='centroids',polygon=Fellows_Creek)
    domain.set_quantity('friction', mannings, location='centroids',polygon=Arts_Center_Creek)
    domain.set_quantity('friction', mannings, location='centroids',polygon=Union_Creek)
    domain.set_quantity('friction', mannings, location='centroids',polygon=Dedman_Pond)
    domain.set_quantity('friction', mannings, location='centroids',polygon=Toad_Creek)   
...
if RAIN_FALL:
    ...
    elif RAIN_EVENT =='20pc':
        rainfall = anuga.file_function(join('03_FORCEFUNC','20pc_des_rain.tms'), quantities=['rate'])
    elif RAIN_EVENT =='10pc':
        rainfall = anuga.file_function(join('03_FORCEFUNC','10pc_des_rain.tms'), quantities=['rate'])
    elif RAIN_EVENT =='05pc':
        rainfall = anuga.file_function(join('03_FORCEFUNC','05pc_des_rain.tms'), quantities=['rate'])
    elif RAIN_EVENT =='02pc':
        rainfall = anuga.file_function(join('03_FORCEFUNC','02pc_des_rain.tms'), quantities=['rate'])
    elif RAIN_EVENT =='01pc':
        rainfall = anuga.file_function(join('03_FORCEFUNC','01pc_des_rain.tms'), quantities=['rate'])
    elif RAIN_EVENT =='500y':
...
if DRAINING:
    ...
    DRAIN_BARRY_DRIVE = anuga.Inlet_operator(domain, Drain_Barry_Polygon,   Q =-1.0)
    DRAIN_LIBRARY     = anuga.Inlet_operator(domain, Drain_Library_Polygon, Q =-1.0)
