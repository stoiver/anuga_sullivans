...
duration = 1 # 8*3600  # duration in seconds
yieldstep = 1# 300 # 5 min
Start_Tide = 558.0
RAIN_FALL = True
...
if myid == 0: print len(interior_holes)
...
if myid == 0:
    ...
    # Cutout bridges
    #-------------------------------------------------------------------------------
    domain.set_quantity('elevation', 560.3, location='centroids',polygon=Union_Bridge)
    domain.set_quantity('elevation', 561.3, location='centroids',polygon=Barry_Drive)
...
if RAIN_FALL == True:
    ...
    if RAIN_EVENT in EVENT_OPTIONS :
        RAIN1 = anuga.Rate_operator(domain, rate=50, factor=1.0e-3, relative_time=False, \
